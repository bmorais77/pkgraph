/**
  * Adapted from org.apache.spark.util.collection.GraphXPrimitiveKeyOpenHashMap.scala
  */
...
class PrimitiveHashMap[
  ...
  val MAX_CAPACITY: Int = 1 << 30
  val INVALID_POS: Int = -1
  val NONEXISTENCE_MASK: Int = 1 << 31
  val POSITION_MASK: Int = (1 << 31) - 1
  ...
  def update(k: K, v: V): Unit = {
    val pos = keySet.addWithoutResize(k) & POSITION_MASK
    _values(pos) = v
  ...
  def setMerge(k: K, v: V, mergeF: (V, V) => V): Unit = {
    ...
    val ind = pos & POSITION_MASK
    if ((pos & NONEXISTENCE_MASK) != 0) { // if first add
      _values(ind) = v
    } else {
  ...
  def changeValue(k: K, defaultValue: => V, mergeValue: (V) => V): V = {
    ...
    if ((pos & NONEXISTENCE_MASK) != 0) {
      ...
      _values(pos & POSITION_MASK) = newValue
      keySet.rehashIfNeeded(k, grow, move)
      newValue
    } else {
  ...
  protected var grow: Int => Unit = (newCapacity: Int) => {
    _oldValues = _values
    _values = new Array[V](newCapacity)
  }

  protected var move: (Int, Int) => Unit = (oldPos: Int, newPos: Int) => {
