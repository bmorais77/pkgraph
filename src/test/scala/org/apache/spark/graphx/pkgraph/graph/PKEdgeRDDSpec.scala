...
class PKEdgeRDDSpec extends FlatSpec with SparkSessionTestWrapper {
  ...
  it should "filter edges and vertices" in {
    val rdd = PKEdgeRDD.fromEdges[Int, Int](sc.parallelize(edges)).mapEdgePartitions { (_, partition) =>
  ...
  it should "inner join with another RDD" in {
    val rdd1 = PKEdgeRDD.fromEdges(sc.parallelize(edges.filter(_.srcId <= 6)))
    val rdd2 = PKEdgeRDD.fromEdges(sc.parallelize(edges.filter(_.srcId >= 4)))
